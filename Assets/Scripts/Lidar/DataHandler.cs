    ...
    public void UpdateData()
    {
        if(data.Count == simulationData.Count)
        {
            for(int i=0;i<data.Count;i++)
            {
                ...
                float _y = data[i].gameObject.transform.position.y  + (float)data[i].levelY + simulationWorld.transform.position.y;

                float _z = data[i].gameObject.transform.position.z + (float)data[i].levelZ + simulationWorld.transform.position.z;
